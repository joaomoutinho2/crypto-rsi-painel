...
def analisar_oportunidades(exchange, moedas):
    ...
    moedas = moedas[:30]  # ⚠️ REDUÇÃO para 30 moedas
    ...
    for moeda in moedas:
        try:
            ...
            del df, entrada, candles, macd_obj, bb  # ⚠️ Limpeza
            gc.collect()
...
def thread_bot():
    ...
    try:
        ...
        exchange.load_markets()
        moedas = [s for s in exchange.symbols if s.endswith("/USDT")][:30]  # ⚠️ LIMITE AQUI

        atualizar_precos_de_entrada(exchange, limite=20)  # ⚠️ REDUZIDO
        atualizar_documentos_firestore(limite=20)         # ⚠️ REDUZIDO
        ...
        while True:
            ...
            for _ in range(6):  # ⏱️ TOTAL 10 minutos com pausas
                time.sleep(100)
                gc.collect()
